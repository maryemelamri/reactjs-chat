...
const makePlagiarismCheck = async () => {
  ...
  const burp0_data = {
    ...
    title: '',
    text: "Eminem se met à attirer plus d'attention autour de lui en développant Slim Shady, un alter ego violent et sadique qui permet au rappeur d'exprimer sa colère avec des paroles traitant notamment de drogues, de viol et de meurtre"
  };
  try {

    const response = await axios.post(burp0_url, burp0_data, {
      headers: burp0_headers,
    cookies: burp0_cookies, // axios does not directly support setting cookies, so you may need to use a cookie jar library for this
      withCredentials: false, // This ensures that cookies are included in the request
      responseType: 'json'
    }); 
    ...
    console.log(" Wttttttttttttttffffuck : " + str);
     result = JSON.parse(str.text);
